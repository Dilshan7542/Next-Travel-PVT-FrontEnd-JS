   ...
   visibleAllFields(evt){
        ...
        promise.then((resp) =>{
            ...
            for (let vehicle of resp.body) {
              $(".fragment-05-main-section").append(` <figure><i>${vehicle.vehicleID}</i> <img src="data:image/png;base64,${vehicle.image}" alt=""><figcaption>${vehicle.brandName}</figcaption> <div><span></span></div></figure>`);
                $(".fragment-05-main-section >:last-child").click(function (){
                    ...
                    sessionStorage.setItem("selectVehicle",$(this).children().eq(0).text())
